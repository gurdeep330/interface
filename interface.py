#!/usr/bin/python3.8
'''To determine interfaces in a PDB complex'''

'''
This script is largely inspired from the works of Francesco Raimondi in
determining GPCR/G-protein interfaces.
For a given pair of chain-IDs of a PDB complex, this script calculates all
possible interfaces between them. Then it maps the interfaces to their
equivalent positions in their native protein's FASTA sequence (from UniProt).
'''
...
## Parse the input
path = os.getcwd()
parser = argparse.ArgumentParser(description='Find Interfaces between given '+
                                'pair of chains in a PDB complex',
                                epilog='Contact: gurdeep330[at]gmail[dot]com')
parser.add_argument('pdb', help='Path to the PDB file (.pdb format)')
...
## Download the PDB complex
print ('Fetching the PDB complex')
if os.path.isfile(tempdir+pdb+'.pdb') == False:
    os.system('wget https://files.rcsb.org/view/'+pdb+'.pdb -O '+tempdir+pdb+'.pdb')
...
def main():
    ...
    for model in structure:
        if given_chainA in model and given_chainB in model:
            ...
            print ('Either or both of the given chains are absent from the PDB complex')
            sys.exit()
        ...
        for accA in dic[chainA.id]:
            for interfaceA in dic_interface:
                pdb_positionA = map_fasta[chainA.id][interfaceA]
                #pdb_positionA = dic_interface[chainA.id][interfaceA].fasta_positionA[interfaceA]
                if pdb_positionA in pdb_to_unpA[accA] and pdb_positionA != '':
                    unp_positionA = pdb_to_unpA[accA][pdb_positionA]
                    #print (dic_interface[chainA.id][interfaceA].residueB)
                    for accB in dic[chainB.id]:
                        for interfaceB in dic_interface[interfaceA].residueB:
                            pdb_positionB = map_fasta[chainB.id][interfaceB]
                            if pdb_positionB in pdb_to_unpB[accB] and pdb_positionB != '':
                                unp_positionB = pdb_to_unpB[accB][pdb_positionB]
                                #print (pdb+'/'+str(interfaceA)+'\t'+str(pdb_positionA)+'\t'+accA+'/'+str(unp_positionA)+'\t'+pdb+'/'+str(interfaceB)+'\t'+str(pdb_positionB)+'\t'+accB+'/'+str(unp_positionB))
                                l += pdb+'/'+chainA.id+'/'+str(interfaceA)+'\t'+str(pdb_positionA)+'\t'+accA+'/'+str(unp_positionA) + '\t'
                                l += pdb+'/'+chainB.id+'/'+str(interfaceB)+'\t'+str(pdb_positionB)+'\t'+accB+'/'+str(unp_positionB) + '\n'
                            else:
                                l += pdb+'/'+chainA.id+'/'+str(interfaceA)+'\t'+str(pdb_positionA)+'\t'+accA+'/'+str(unp_positionA) + '\t'
                                l += pdb+'/'+chainB.id+'/'+str(interfaceB)+'\t'+str('-')+'\t'+accB+'/'+str('-') + '\n'
                    ...
                    for accB in dic[chainB.id]:
                        for interfaceB in dic_interface[interfaceA].residueB:
                            ...
                            if pdb_positionB in pdb_to_unpB[accB] and pdb_positionB != '':
                                unp_positionB = pdb_to_unpB[accB][pdb_positionB]
                                #print (pdb+'/'+str(interfaceA)+'\t'+str(pdb_positionA)+'\t'+accA+'/'+str(unp_positionA)+'\t'+pdb+'/'+str(interfaceB)+'\t'+str(pdb_positionB)+'\t'+accB+'/'+str(unp_positionB))
                                l += pdb+'/'+chainA.id+'/'+str(interfaceA)+'\t'+str('-')+'\t'+accA+'/'+str('-') + '\t'
                                l += pdb+'/'+chainB.id+'/'+str(interfaceB)+'\t'+str(pdb_positionB)+'\t'+accB+'/'+str(unp_positionB) + '\n'
                            else:
                                l += pdb+'/'+chainA.id+'/'+str(interfaceA)+'\t'+str('-')+'\t'+accA+'/'+str('-') + '\t'
                                l += pdb+'/'+chainB.id+'/'+str(interfaceB)+'\t'+str('-')+'\t'+accB+'/'+str('-') + '\n'
